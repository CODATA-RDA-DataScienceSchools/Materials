...
We will use [Google Colab](https://colab.google/)
...
- [tmdb-movies.missing.csv](https://github.com/CODATA-RDA-DataScienceSchools/Materials/blob/master/docs/DataTrieste2025/Python/data/tmdb-movies-missing.csv)
- [movies.csv](https://github.com/CODATA-RDA-DataScienceSchools/Materials/blob/master/docs/DataTrieste2025/Python/data/movies.csv)
- [ratings.csv](https://github.com/CODATA-RDA-DataScienceSchools/Materials/blob/master/docs/DataTrieste2025/Python/data/ratings.csv)
...
-   Pressing the [+Code]{.red} button in the menu bar will add a new code cell.
...
-   Sometimes there are a few lines of metadata at the top of the file:
    ...
    -   You can use skip the first "n" lines: `read_csv("data.csv", skiprows=5)`
    -   Or drop all lines that start with a certain character, e.g. "\#": `read_csv("data.csv", comment="#")`

-   The data might not have column names:

    -   You can pass `usecols` a list with column names: `pd.read_csv("data.csv", usecols=['foo', 'bar'])[['foo', 'bar']]`

-   Specify additional value(s) that are used to represent missing values in your file: `pd.read_csv("data.csv", na_values=".")`

-   Trim whitespace: `pd.read_csv("data.csv", skipinitialspace=True)`
...
![](images/concat.png){fig-align="center" width="600"}

## Time for... {.hcenter}

![](images/Practical.PNG){fig-align="center"}

## Joining dataframes {.hcenter}
...
## Writing your own functions {.hcenter .smaller}
...
variable_counts = count_variable_occurrences(file_path, 'variable_name')
