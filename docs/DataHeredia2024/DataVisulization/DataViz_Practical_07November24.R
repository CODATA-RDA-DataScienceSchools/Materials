...
mybar <-  ggplot(gapminder, aes(x=continent, fill=continent)) + 
  geom_bar(aes(y=after_stat(count)/12)) +
...
gapbox <- ggplot(data=gapminder, aes(x=continent, y=lifeExp, fill=continent, guides(fill= FALSE)))
gapbox +
  geom_boxplot(outlier.size=2, show.legend =FALSE)
#2. Also, make the plot horizontal
gapbox +
  geom_boxplot(outlier.size=2, show.legend =FALSE) + coord_flip()
...
gdpbox<- ggplot(data=gapminder, aes(x=gdpPercap, fill=continent)) +
  geom_density(alpha=0.3)
gdpbox
#2.  plot GDP on a log scale
gdpbox<- ggplot(data=gapminder, aes(x=log10(gdpPercap), fill=continent)) +
  geom_density(alpha=0.3)
gdpbox
#3. Make boxplots of gdpPercap by continent
gdpbox<- ggplot(data=gapminder, aes(x=continent, y=gdpPercap, fill=continent, guides(fill= FALSE)))
gdpbox +
  geom_boxplot(outlier.size=2, show.legend =FALSE)
#4. Do the same, but plot GDP on a log scale
gdpbox<- ggplot(data=gapminder, aes(x=continent, y=log10(gdpPercap), fill=continent, guides(fill= FALSE)))
gdpbox +
  geom_boxplot(outlier.size=2, show.legend =FALSE) + coord_flip()
...
indiachina <- subset(gapminder, country=="India" | country=="China")
indiachinaplot <- ggplot(indiachina, aes(y=lifeExp, x=gdpPercap))
indiachinaplot + geom_line() + geom_point(aes(color=country))
indiachinaplot+ aes(color=country) + geom_line() + geom_point() #making color global
